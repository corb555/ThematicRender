...
The Coordinator is the central orchestrator. It is the sole owner of overall daemon state and is the only component that makes decisions about request acceptance, request lifecycle, batch execution, and subsystem coordination.

The worker processes perform rendering work only. They have no knowledge of the Editor, socket protocol, or JSON message format.
...
* `Command_rcv` receives socket messages, performs straightforward syntax validation, and forwards normalized commands to the Coordinator
...
Reader, Renderer, and Writer workers operate only on internal envelopes and work packets. They do not know that an Editor exists, and they do not know anything about socket format, JSON structure, or request/response protocol.
...
`Command_rcv` shall not decide whether a request is valid in the broader orchestration sense. That remains the Coordinator’s responsibility.
...
A response may be triggered by an event that arrived through `status_queue`, such as an error from a worker, but the response itself is an outbound message and does not return through the inbound event path.
...
If `Command_rcv` encounters a read failure, disconnect, or invalid inbound message, it shall forward a normalized error or disconnect event to `status_queue`.
