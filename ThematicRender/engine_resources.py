...
from ThematicRender.ipc_packets import JOB_ID_IDLE
...
class EngineResources:
    ...
    def _initialize_shm_pools(self, input_slots: int, num_renderers: int, num_readers: int, buffer_factor: float) -> None:
        """
        ...
        out_spec = PoolSpec(
            data_shape=(3, 256, 256), data_dtype=np.dtype(np.uint8),
            mask_shape=(1, 256, 256), mask_dtype=np.dtype(np.float32)
        )
        ...
        static_count, transit_count = calculate_shm_partitions(
            total_slots=input_slots,
            num_renderers=num_renderers,
            num_readers=num_readers, # Added
        ...
        print(f"   - Transit Highway:  {transit_count:4} slots (Capacity for {buffer_factor}x load)")
        print(f"          - Load:             {load} ({num_renderers} Renderers × {num_drivers} Drivers + {num_readers} Readers)")
        print(f"   - Static Cache:     {static_count:4} slots (Remaining slots pinned for Warm Previews)\n")
        print(f"   - Output Highway:   {out_slots:4} slots (Write-buffer for {num_renderers} Renderers @ {buffer_factor}x)")
        print(f"\n   💡 [Optimization Tips]:")
        if static_count < (input_slots * 0.2):
            print(f"   ⚠️  Warning: Static Cache is very small (<20%). Previews may be slow.")
            print(f"      To fix: Increase 'input_slots' in system.yml to at least {transit_count + 100}.")
        else:
            print(f"   ✅  Static Cache is healthy.")

        if transit_count < (num_renderers * num_drivers):
            print(f"   🛑 Critical: Transit Highway is under-sized! Potential for pipeline deadlock.")
            print(f"      To fix: Increase 'input_slots' or decrease 'transit_buffer_factor'.")
        else:
            print(f"   ✅  Transit Highway is healthy.")

        print(f"   - To change Transit Highway, adjust 'system.yml:transit_buffer_factor' (Current: {buffer_factor})")
    ...
    def set_engine_idle(self):
        ...
        if self.ctx_store:
            self.ctx_store.set_job_id(JOB_ID_IDLE) # Assuming you add a generic setter
...
def calculate_shm_partitions(
    total_slots: int,
    num_renderers: int,
    num_readers: int, # New parameter
    num_drivers: int,
    buffer_factor: float = 2.0,
    max_transit_ratio: float = 0.8
) -> Tuple[int, int]:
    """
